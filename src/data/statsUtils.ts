...
export const calculateDashboardStats = (responses: SurveyResponse[]): DashboardStats => {
  ...
  const sortedForNps = [...npsResponses].sort((a, b) => new Date(a.submittedAt).getTime() - new Date(b.submittedAt).getTime());
  ...
  // Response rate: compare current period vs previous period of data based on time
